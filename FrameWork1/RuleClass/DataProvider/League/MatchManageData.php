...
class MatchManageData extends BaseManageData
{
     ...
     * @param int $isKnockout 是否淘汰赛
     * @return array 数据集
     */
    public function GetListFinishedOfLeague($leagueId,$isKnockout=-1) {
        $result=-1;
        if($leagueId>0){
            $knockoutSelection="";
            if($isKnockout>=0){
                $knockoutSelection=" AND IsKnockout=$isKnockout ";
            }
            $dataProperty = new DataProperty();
            $dataProperty->AddField("LeagueId", $leagueId);
            ...
            $sql = "
                ...
                WHERE m.LeagueId=:LeagueId AND m.State=2 AND m.Result>0 $knockoutSelection;";
