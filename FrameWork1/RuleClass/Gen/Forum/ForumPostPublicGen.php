...
class ForumPostPublicGen extends ForumBasePublicGen implements IBasePublicGen
{
    ...
    private function GenList()
    {
        ...
        $replayCount = $forumPostPublicDate-> GetReplayCount($forumTopicId, 0);
        $hidCount   =  $forumPostPublicDate-> GetHitCount($forumTopicId, 0);
        $forumPostPublicDate->AddHitCount($forumTopicId, $replayCount, $hidCount);
    ...
    private function GenCreate()
    {
        ...
        if (!empty($_POST)) {
            ...
            if ($forumPostId > 0) {
                ...
                //同步到ForumTopic表
                $replyCount = $forumTopicPostData -> GetReplayCount($forumTopicId, 0);
                $forumTopicPublicData -> SyncParentForumTopicInfoWhenCreate(
                    $forumTopicId,
                    $forumPostId,
                    $userId,
                    $userName,
    ...
    private function GenModify()
    {
        ...
        if ($userId > 0) {
            ...
            $forumPostPublicData = new ForumPostPublicData();
            $forumUserId = $forumPostPublicData->GetUserId($forumPostId, false);


            if ($forumUserId == $userId) { //自己的帖子
                //读取权限
                $can = parent::CheckUserPopedom(UserPopedomData::ForumEditSelfPost);
            }
            else {
                $can = parent::CheckUserPopedom(UserPopedomData::ForumEditOtherPost);
            ...
            if ($can) {
                ...
                if (!empty($_POST)) {
                    ...
                    $forumPostContent = Control::PostRequest("f_ForumPostContent", "", false);
                    $forumPostTitle = Control::PostRequest("f_ForumPostTitle", "", false);
                    $forumPostContent = str_ireplace('\"', '"', $forumPostContent);
                    //内容中不允许脚本等
                    $forumPostContent = Format::RemoveScript($forumPostContent);
                    $forumPostModify = new ForumPostPublicData();

                    $result = $forumPostModify->ModifyContent($forumPostId, $forumPostTitle, $forumPostContent);

                    if ($result > 0) {
                        Control::GoUrl("/default.php?mod=forum_post&a=list&forum_topic_id=$forumTopicId");
                    }
                    else{
                        Control::GoUrl("www.baidu.com");
                    }
                }
