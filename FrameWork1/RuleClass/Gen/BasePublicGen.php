...
class BasePublicGen extends BaseGen
{
    ...
    /**
     * 接受多个权限参数,检查这些权限是否都被具备,全部具备则返回true
     * @return bool
     */
    protected function CheckUserPopedom()
    {
        $arg =  func_get_args();

        $userPopedomList = self::GetArrayOfUserPopedom();

        foreach($arg as $item)
        {
            if(!empty($userPopedomList) && in_array($item, $userPopedomList)){
                continue;
            }
            else{
                return false;
     ...
     * @return array
     */
    private function GetArrayOfUserPopedom()
    {
        $result = array();

        $siteId = self::GetSiteIdByDomain();
        $userId = Control::GetUserId();
        $userRolePublicData =  $userRolePublicData = new UserRolePublicData();
        $userGroupId = $userRolePublicData->GetUserGroupId(
            $siteId,
            $userId,
            false
        ...
        if($siteId > 0 && $userId > 0){
            $userPopedomPublicData = new UserPopedomPublicData();
            //$popedomOfUser = $userPopedomPublicData->GetArrayListOfUserPopedom($userId, $siteId, 1, false);
            //TODO 获取单个用户权限与组权限的合并
            $popedomOfUserGroup = $userPopedomPublicData->GetArrayListOfUserGroupPopedom($userGroupId, $siteId, 1, false);
            if(!empty($popedomOfUserGroup)){
                foreach($popedomOfUserGroup as $item){
                    $result[] = $item['UserPopedomName'];
                }
            }
        }
