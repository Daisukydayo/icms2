...
class ForumPublicData extends BasePublicData {
    ...
    /**
     * @param $forumId                  int     板块id
     * @param $lastForumTopicIdInDb     int     数据库中保存的最新帖子的id
     * @param $lastForumTopicIdDelete   int     被删除帖子的id
     * @param $lastPostInfo             int     ?条最后发布综合信息
     */
    public function UpdateForumInfoWhenDeleteOneTopic($forumId, $lastForumTopicIdInDb, $lastForumTopicIdDelete,$lastPostInfo)
    {
        $result = -1;
        if($forumId > 0){
            $dataProperty = new DataProperty();
            ...
            if($lastForumTopicIdInDb == $lastForumTopicIdDelete){  //被删除的帖子刚好是最新的帖子
                $sql = 'UPDATE ' .self::TableName_Forum.
                        ' SET LastForumTopicId=0,'.
                            ' LastForumTopicTitle="",'.
                            ' TopicCount=(
                                          SELECT COUNT(ForumTopicId) FROM '.self::TableName_ForumTopic.
                                          ' WHERE ForumId=:ForumId AND State< ' .ForumTopicData::FORUM_TOPIC_STATE_REMOVED.
                                          '),'.
                            ' PostCount=(
                                          SELECT COUNT(ForumPostId) FROM ' .self::TableName_ForumPost.
                                          ' WHERE ForumId=:ForumId AND State<' .ForumTopicData::FORUM_TOPIC_STATE_REMOVED.
                                          '),'.
     ...
     * @return array
     */
    public function GetListByForumIdWithoutState($forumId) {
