...
//                $dataProperty->AddField("SiteId3", $siteId);
     ...
     * @return string url
     */
    public function GetCollectChannelId($channelId, $withCache)
    {
        $result = -1;
        if ($channelId > 0) {
            $cacheDir = CACHE_PATH . DIRECTORY_SEPARATOR . 'channel_data';
            $cacheFile = 'channel_get_collect_channel_id.cache_' . $channelId . '';
            $sql = "SELECT CollectChannelId FROM " . self::TableName_Channel . " WHERE ChannelId=:ChannelId;";
            $dataProperty = new DataProperty();
            $dataProperty->AddField("ChannelId", $channelId);
            $result = $this->GetInfoOfStringValue($sql, $dataProperty, $withCache, $cacheDir, $cacheFile);
        }
    ...
    /**
     * 聚合排序号频道ID(用于在所输入的频道ID中取得最大排序号的值，适应于多个频道聚合的节点使用，id用半角,分隔)
     * @param int $channelId 频道id
     * @param bool $withCache 是否从缓冲中取
     * @return string url
     */
    public function GetSortChannelId($channelId, $withCache)
    {
        $result = -1;
        if ($channelId > 0) {
            $cacheDir = CACHE_PATH . DIRECTORY_SEPARATOR . 'channel_data';
            $cacheFile = 'channel_get_sort_channel_id.cache_' . $channelId . '';
            $sql = "SELECT SortChannelId FROM " . self::TableName_Channel . " WHERE ChannelId=:ChannelId;";
            $dataProperty = new DataProperty();
            $dataProperty->AddField("ChannelId", $channelId);
            $result = $this->GetInfoOfStringValue($sql, $dataProperty, $withCache, $cacheDir, $cacheFile);
        }
