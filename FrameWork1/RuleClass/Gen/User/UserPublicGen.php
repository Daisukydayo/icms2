...
class UserPublicGen extends BasePublicGen implements IBasePublicGen
{
    ...
    private function AsyncRegister()
    {
        ...
        if ($siteId > 0 && (!empty($userName) || !empty($userEmail) || !empty($userMobile)) && !empty($userPass) && !empty($regIp)) {
            ...
            if (!empty($userMobile)) {
                if (preg_match("/^13\d{9}$|^15\d{9}$|^18\d{9}$|^17\d{9}$/", $userMobile)) {
                    $isSameMobile = self::IsRepeatUserMobile($userMobile);
    ...
    private function WeiXinEnter(){
        ...
        //验证token码
        $checkSignature=self::WeiXinCheckSignature();


        if($checkSignature){

            //接受用户发送的微信消息
            $arrayWeiXinMessageInfo=self::WeiXinGetMessage();

            if(!empty($arrayWeiXinMessageInfo)){
                $wx_ToUserName=$arrayWeiXinMessageInfo["ToUserName"];
                $wx_FromUserName=$arrayWeiXinMessageInfo["FromUserName"];
                $wx_CreateTime=$arrayWeiXinMessageInfo["CreateTime"];
                $wx_MsgType=$arrayWeiXinMessageInfo["MsgType"];
                $wx_Content=$arrayWeiXinMessageInfo["Content"];
                $wx_MsgId=$arrayWeiXinMessageInfo["MsgId"];

                //由user发来消息的前置编码分配具体事件  编码与参数暂有井号“#”隔开:  事件编码#参数
                $arr=explode("#",$wx_Content);
                $action=strtolower($arr[0]);
                $content=$arr[1];
                switch($action){
                    case "cg":  //change group
                        self::WeiXinChangeGroup($content,$wx_FromUserName);
                        break;
                }
            }
        }
     ...
     *
     * 微信换会员组
     * @param $groupName string 要换到的组名
     * @param $openId string openid
     */
    private function WeiXinChangeGroup($groupName,$openId){
        ...
        //先取token码
        $accessToken=self::GetWxAccessToken();
        $urlGetGroupJson = "https://api.weixin.qq.com/cgi-bin/groups/get?access_token=$accessToken";

        $ch_getGroup=curl_init();
        curl_setopt($ch_getGroup, CURLOPT_URL, $urlGetGroupJson);
        curl_setopt($ch_getGroup, CURLOPT_RETURNTRANSFER, 1);//不直接输出，返回到变量
        $groupInfoJson = curl_exec($ch_getGroup);
        curl_close($ch_getGroup);
        $groupInfoArray=json_decode($groupInfoJson,true);

        foreach($groupInfoArray["groups"] as $oneGroup){
            if($oneGroup["name"]==$groupName){
                ...
                $toGroupId=$oneGroup["id"];
                $urlGetAccessToken = "https://api.weixin.qq.com/cgi-bin/groups/members/update?access_token=$accessToken";
                //执行 http 请求，返回 json
                $weiXinPost='{"openid":"'.$openId.'","to_groupid":'.$toGroupId.'}';
                $ch_update = curl_init();
                curl_setopt($ch_update, CURLOPT_URL, $urlGetAccessToken);
                curl_setopt($ch_update, CURLOPT_POST, 1);
                curl_setopt($ch_update, CURLOPT_POSTFIELDS, $weiXinPost);
                //curl_setopt($ch_update, CURLOPT_RETURNTRANSFER, 1);//不直接输出，返回到变量
                $updateResult=curl_exec($ch_update);
                curl_close($ch_update);

                if($updateResult){
                    self::WeiXinReturnString($openId,"您好！感谢您关注长沙女性，这里是长沙市妇女联合会官方微信。我们始终致力于关注女性、关爱儿童、关怀家庭，与长沙几百万妇女姐妹一起，贡献“她智慧”、释放“她力量”、书写“她时代”。");
                }else{
                    self::WeiXinReturnString($openId,"您好！感谢您关注长沙女性，这里是长沙市妇女联合会官方微信。");
                }
                break;
            }
        }
     ...
     * 验证本地token
     */
    private function WeiXinCheckSignature()
    {
        //比对token码
     ...
     * 取得用户微信消息内容
     */
    private function WeiXinGetMessage(){
        //取消息
        /**
         *
         * 微信xml格式:
    ...
    /**
        ...
        *@return string
    */
    public function WeiXinReturnString($openId,$content){
        $wx_name="cswomen888";
        $createTime=time();

        $xmlText="<xml>
                  <ToUserName><![CDATA[".$openId."]]></ToUserName>
                  <FromUserName><![CDATA[".$wx_name."]]></FromUserName>
     ...
     * test weixin
     */
    private function TestWeiXin(){

            echo "11";
            $urlGetAccessToken = "https://api.weixin.qq.com/cgi-bin/groups/get?access_token=SGECjU_SDRTGSZzrPudJFWUuKwS0pxv0Mi9DIuDPa0XX_YDXj_g2nAvEmhwakgyeIEfMC-Amx4TgvZ8WpTzxhNNFu0v5edJA2o0Xrw_dpphC8chNRZTbl1qAeBsBOt_4PVXjAGAVBD";

            //执行 http 请求，返回 json
            $ch = curl_init();
            //curl_setopt($ch, CURLOPT_POST, 1);
            //curl_setopt($ch, CURLOPT_POSTFIELDS, $weiXinPost);
            //curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);//不直接输出，返回到变量
        curl_setopt($ch, CURLOPT_URL, $urlGetAccessToken);
        ...
        $a = curl_exec($ch);//$output=curl_exec($ch);
            curl_close($ch);
        echo "fo";
            var_dump($a);
        //var_dump($_POST);
