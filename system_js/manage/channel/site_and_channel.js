 ...
 * 频道类型 聚合频道 17
 */
window.CHANNEL_TYPE_COLLECTION = 17;
/**
 * 频道类型 外部接口类 50
 */
...
function _ChannelClick() {
    if (window.G_SelectedChannelId > 0) {
        ...
        if (window.G_SelectedChannelType === window.CHANNEL_TYPE_DOCUMENT_NEWS) { //新闻类频道
            ...
            addTab();
        } else if (window.G_SelectedChannelType === window.CHANNEL_TYPE_COLLECTION) { //聚合频道

            window.G_TabTitle = "";
            window.G_TabUrl = '/default.php?secu=manage&mod=document_news&m=list_of_collection&channel_id=' + window.G_SelectedChannelId;
            addTab();
