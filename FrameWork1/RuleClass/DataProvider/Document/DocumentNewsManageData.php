...
class DocumentNewsManageData extends BaseManageData
{
    ...
    )
    {
        ...
        $sql = "
            ...
            FROM
            " . self::TableName_DocumentNews . "
            WHERE ChannelId IN ($channelIds) AND State<100 " . $searchSql . " " . $conditionManageUserId . "
            ORDER BY $hitSortMethod Sort $sortMethod, CreateDate DESC LIMIT " . $pageBegin . "," . $pageSize . ";";

        $result = $this->dbOperator->GetArrayList($sql, $dataProperty);

        $sql = "SELECT count(*) FROM " . self::TableName_DocumentNews . "
                WHERE ChannelId IN ($channelIds) AND State<100 " . $conditionManageUserId . " " . $searchSql . ";";
        $allCount = $this->dbOperator->GetInt($sql, $dataProperty);
     ...
     * @param string $sortChannelId 聚合排序号频道ID
     * @return int 影响的记录行数
     */
    public function ModifySortWhenCreate($channelId, $documentNewsId, $sortChannelId = '') {
        $result = -1;
        if($channelId >0 && $documentNewsId>0){

            if (strlen($sortChannelId) > 0){
                $sortChannelId = Format::FormatSql($sortChannelId);
                $sql = "SELECT max(Sort) FROM " . self::TableName_DocumentNews . " WHERE ChannelId IN ($sortChannelId);";
                $maxSort = $this->dbOperator->GetInt($sql, null);
            }else{
                $dataProperty = new DataProperty();
