(function(f,a,c,g,e){
    ...
    function b(i){
        ...
        function d(i){
        i.html('<div class="plupload_wrapper"><div class="ui-widget-content plupload_container"><div class="plupload"><div class="ui-state-default ui-widget-header plupload_header"><div class="plupload_header_content"><div class="plupload_header_title">'+b("请选择要上传的文件")+'</div><div class="plupload_header_text">'+b("添加文件至上传队列，然后点击开始上传按钮")+'</div></div></div><div class="plupload_content"><table class="plupload_filelist"><tr class="ui-widget-header plupload_filelist_header"><td class="plupload_cell plupload_file_name">'+b("文件名")+'</td><td class="plupload_cell plupload_file_status">'+b("状态")+'</td><td class="plupload_cell plupload_file_size">'+b("大小")+'</td><td class="plupload_cell plupload_file_action">&nbsp;</td></tr></table><div class="plupload_scroll"><table class="plupload_filelist_content"></table></div><table class="plupload_filelist"><tr class="ui-widget-header ui-widget-content plupload_filelist_footer"><td class="plupload_cell plupload_file_name"><div class="plupload_buttons"><!-- Visible --><a class="plupload_button plupload_add">'+b("添加文件")+'</a>&nbsp;<a class="plupload_button plupload_start">'+b("开始上传")+'</a>&nbsp;<a class="plupload_button plupload_stop plupload_hidden">'+b("停止上传")+'</a>&nbsp;</div><div class="plupload_started plupload_hidden"><!-- Hidden --><div class="plupload_progress plupload_right"><div class="plupload_progress_container"></div></div><div class="plupload_cell plupload_upload_status"></div><div class="plupload_clearer">&nbsp;</div></div></td><td class="plupload_file_status"><span class="plupload_total_status">0%</span></td><td class="plupload_file_size"><span class="plupload_total_file_size">0 kb</span></td><td class="plupload_file_action"></td></tr></table></div></div></div><input class="plupload_count" value="0" type="hidden"></div>')
        }
        ...
        if(m){
            ...
            switch(p.code){
                case c.FILE_EXTENSION_ERROR:
                    m=b("文件: %s").replace("%s",n.name);
                    break;
                case c.FILE_SIZE_ERROR:
                    m=b("文件 : %f, 大小: %s, 最大值: %m").replace(/%([fsm])/g,function(r,q){
                    switch(q){
                        case"f":
                            return n.name;
                        case"s":
            ...
            case i.FILE_COUNT_ERROR:
                m=b("一次只允许 %d 个文件上传，多余的文件未添加到队列中").replace("%d",i.options.max_file_count);
                break;
...
_handleState:function(){
    ...
    if(j.state===c.STARTED){
        ...
        g(".plupload_upload_status",i.element).text(b("已上传 %d/%d 个文件").replace("%d/%d",j.total.uploaded+"/"+j.files.length));
...
_updateTotalProgress:function(){
    ...
    g(".plupload_upload_status",this.element).text(b("已上传 %d/%d 个文件").replace("%d/%d",i.total.uploaded+"/"+i.files.length))
    },
_updateFileList:function(){
    ...
    if(n.total.queued===0){
        g(".ui-button-text",j.browse_button).text(b("添加文件"))
        }else{
        g(".ui-button-text",j.browse_button).text(b("%d 个文件正在队列中").replace("%d",n.total.queued))
        }
    ...
    if(!n.files.length&&n.features.dragdrop&&n.settings.dragdrop){
        g("#"+o+"_filelist").append('<tr><td class="plupload_droptext">'+b("拖动文件到这里")+"</td></tr>")
        }else{
...
_enableDragAndDrop:function(){
    this.filelist.append('<tr><td class="plupload_droptext">'+b("拖动文件到这里")+"</td></tr>");
...
_notify:function(j,k){
    var i=g('<div class="plupload_message"><span class="plupload_message_close ui-icon ui-icon-circle-close" title="'+b("关闭")+'"></span><p><span class="ui-icon"></span>'+k+"</p></div>");
