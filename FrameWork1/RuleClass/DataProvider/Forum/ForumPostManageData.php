    ...
    public function GetListPager($forumTopicId, $pageBegin, $pageSize, &$allCount, $searchKey) {
        ...
        if($forumTopicId>0){
            ...
            if (strlen($searchKey) > 0 && $searchKey != "undefined") {
                $searchSql.=" AND fp.ForumPostTitle LIKE :SearchKey ";
                $dataProperty->AddField("SearchKey", "%" . $searchKey . "%");
            }

            $sql = "
                SELECT
                fp.*,u.UserName,u.UserMobile,u.UserEmail
                FROM
                " . self::TableName_ForumPost . " fp,".self::TableName_User." u
                WHERE fp.ForumTopicId=:ForumTopicId " . $searchSql . " AND fp.UserId=u.UserId AND fp.IsTopic=0 ORDER BY fp.PostTime DESC LIMIT " . $pageBegin . "," . $pageSize . " ;";
            ...
            $sql = "
                ...
                FROM
                " . self::TableName_ForumPost . " fp,".self::TableName_User." u
                WHERE fp.ForumTopicId=:ForumTopicId " . $searchSql . " AND fp.UserId=u.UserId AND fp.IsTopic=0;";
