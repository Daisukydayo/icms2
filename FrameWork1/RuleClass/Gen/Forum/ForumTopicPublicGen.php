...
class ForumTopicPublicGen extends ForumBasePublicGen implements IBasePublicGen
{
        ...
        if (!empty($_POST)) {
            ...
            if ($forumTopicId > 0) {
                ...
                if ($forumPostId > 0) {
                    ...
                    $lastPostInfo = Format::AddToInfoString($forumTopicId, $forumTopicTitle, $lastPostInfo, 5);
    ...
    private function GenModify()
    {
        ...
        if($forumId < 0 && $forumPostId < 0 && $siteId <= 0){
            die();
        }
        ...
        if ($forumTopicId > 0 && $userId >= 0) {
            ...
            if ($can) {
                ...
                if ($result > 0) {
                    ...
                    $forumPublicData = new ForumPublicData();
                    //获取被删除了帖子信息
                    $singleForumInfo = $forumPublicData->GetListByForumIdWithoutState($forumId);

                    $lastPostInfoInDb = $singleForumInfo["LastPostInfo"];
                    $lastForumTopicIdInDb = $singleForumInfo["LastForumTopicId"];

                    $lastPostInfo = Format::RemoveOneFromInfoString($forumTopicId ,$lastPostInfoInDb);

                    //更新版块信息
                    $forumPublicData->UpdateForumInfoWhenDeleteOneTopic(
                        $forumId,
                        $lastForumTopicIdInDb,
                        $forumTopicId,
                        $lastPostInfo
                        );
