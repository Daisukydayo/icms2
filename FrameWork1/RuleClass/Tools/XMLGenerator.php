...
class XMLGenerator {
    ...
    /**
     * 生成XML数据（只针对document news表）给一点资讯
     * @param string $channelTitle 频道标题
     * @param string $channelDescription 频道描述
     * @param string $channelLink 频道连接
     * @param string $language 语言
     ...
     * @return string 数据字符串（xml）
     */
    public static function GenForDocumentNewsToYiDianZiXun($channelTitle, $channelDescription, $channelLink, $language, $items = null, $pubDate = "", $lastBuildDate = "", $generator = "", $channelImgUrl = "") {
        $rss = "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>\r\n";
        $rss .= "<rss version=\"2.0\">\r\n";
        $rss .= "<channel>\r\n";
        $rss .= "<title><![CDATA[{$channelTitle}]]></title>\r\n";
        ...
        for ($i = 0; $i < count($items); $i++) {
            ...
            $rss .= "<title><![CDATA[{$items[$i]['DocumentNewsTitle']}]]></title>\r\n";
            $rss .= "<author><![CDATA[{$items[$i]['Author']}]]></author>\r\n";
            $rss .= "<source><![CDATA[{$items[$i]['SourceName']}]]></source>\r\n";
            $rss .= "<link><![CDATA[{$items[$i]['DocumentNewsUrl']}]]></link>\r\n";
            $rss .= "<description><![CDATA[{$items[$i]['DocumentNewsContent']}]]></description>\r\n";
            $rss .= "</item>\r\n";
        }
